...
function calculate1RM(w, r){
    ...
    let epley = (r === 0) ? 0 : (r === 1) ? w : Math.round(w * (1 + (r/30)))
    let Mc = (r === 0) ? 0 : (r === 1) ? w : Math.round((100 * w)/(101.3 - (2.67123 * r)))
    let lombardi = (r === 0) ? 0 : (r === 1) ? w : Math.round(w * (r ** 0.10))
    
    console.log([epley,Mc,lombardi])
    if (epley > Mc && epley > lombardi || epley == lombardi || epley == Mc) {
      return epley
    }  if (Mc > epley && Mc > lombardi || Mc == lombardi || Mc == epley ) {
      return Mc
    }  if ( lombardi > epley && lombardi > Mc || lombardi == epley || lombardi == Mc){
      return lombardi
    }
  }